$(function () {
    ...
    $('#category').change(function () {
        ...
        $selectSubCategory.html(original).find('option').each(function () {
            ...
            if (subCategory != selectCategory_after) {
                $('#subCategory').val("s");
            }
        });
    ...
    $('#subCategory').focus(function () {
        ...
        $selectSubCategory.html(original).find('option').each(function () {
            ...
            if (category != selectCategory_after) {
                $(this).not(':first-child').remove();
            }
        });
    });
...
function input_check() {
    ...
    if (comment == "") {
        $("#comment-error").text("説明文が空白です");
        $("#comment").addClass("input_error");
        result = false;
    }

    if (stock == "") {
        $("#stock-error").text("所持数が空白です");
        $("#stock").addClass("input_error");
        result = false;
    }

    if (weight == "") {
        $("#weight-error").text("重量が空白です");
        $("#weight").addClass("input_error");
        result = false;
    }

    if (power == "") {
        $("#power-error").text("消費電力が空白です");
